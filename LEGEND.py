...
TOKEN = "7733663845:AAFDwnBC-7NOk9BxK6aPEiGP7L-TGtJQUrI"
...
async def approve(update: Update, context: CallbackContext):
    ...
    if len(message_text) > 1:
        user_id = message_text[1]
        approved_users.add(user_id)
        await update.message.reply_text(f"User {user_id} approved!")
    else:
        await update.message.reply_text("Usage: /approve <user_id>")

# Disapprove user command (admin only)
...
# Middleware to check user approval
async def check_approval(update: Update, context: CallbackContext):
    user_id = str(update.message.chat_id)
    if user_id not in approved_users and user_id != str(ADMIN_ID):
        await update.message.reply_text("You are not approved by the admin to use this bot.")
        return False
    return True
def main():
